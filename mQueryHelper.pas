...
begin
  CheckSQLResult( SQLDescribeCol( hstmt,
                                  ...
                                  decdig,
                                  Nullable));
  CheckSQLResult( SQLColAttribute( hstmt,
                                   FFieldNo,
                                   SQL_DESC_AUTO_UNIQUE_VALUE,
                                   nil,SQL_SMALLINT, nil, @AutoInc)
        );
  CheckSQLResult( SQLColAttribute( hstmt,
                                   FFieldNo,
                                   SQL_COLUMN_UPDATABLE{SQL_DESC_UPDATABLE},
                                   nil,SQL_SMALLINT, nil, @Updatable)
        );
  if FFieldNo = 0 then
  begin
//    BookmarkType := dtype;
  ...
  begin
    if (AutoInc=SQL_TRUE) and (Nullable=SQL_NO_NULLS) then Nullable := SQL_NULLABLE;
  end;
  i := 0;
  if pname[0] = #0 then
...
procedure TmCustomQuery.CreateParams(List: TStParams; const Value: PChar);
var
...
procedure TmCustomQuery.SetParamsList(Value: TStParams);
begin
